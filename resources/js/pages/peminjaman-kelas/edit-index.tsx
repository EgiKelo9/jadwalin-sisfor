...
export default function UbahPeminjamanKelas({ peminjamanKelasForm, ruangKelas, peminjamanKelas, user, userRole }: { peminjamanKelasForm: PeminjamanKelasForm, ruangKelas: RuangKelasData[], peminjamanKelas: PeminjamanKelasData[], user: UserData, userRole: string }) {
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        {
            title: 'Ubah',
            href: `/${userRole}/peminjaman-kelas/${peminjamanKelasForm.id}/ubah`,
        },
    ];

    const { data, setData, post, processing, errors } = useForm<Required<PeminjamanKelasForm>>({
    ...
    const handleSubmit: FormEventHandler = (e) => {
        ...
        post(route(`${userRole}.peminjaman-kelas.update`, peminjamanKelasForm.id), {
            forceFormData: true,
            preserveState: true,
            preserveScroll: true,
            onSuccess: () => {
                ...
                router.visit(route(`${userRole}.peminjaman-kelas.edit`, peminjamanKelasForm.id));
            },
            onError: (errors) => {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs} userRole={userRole}>
            <Head title="Ubah Peminjaman Kelas" />
            <ToasterComponent />
            <form className="flex h-full w-full flex-1 flex-col gap-4 rounded-xl p-4 overflow-x-auto" onSubmit={handleSubmit}>
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <h1 className='text-xl py-2 font-semibold'>Ubah Peminjaman Kelas</h1>
                    <Button variant="outline" asChild>
                        <Link href={`/${userRole}/peminjaman-kelas/${peminjamanKelasForm.id}`}><Eye />Lihat</Link>
                    </Button>
                </div>
                {page === 1 && (
