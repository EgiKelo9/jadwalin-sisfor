...
export default function LihatRiwayatPeminjamanKelas({ peminjamanKelas, userRole, canUpdate }: { peminjamanKelas: PeminjamanKelasData, userRole: string, canUpdate?: boolean }) {
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        {
            title: 'Lihat',
            href: `/${userRole}/peminjaman-kelas/riwayat/${peminjamanKelas.id}`,
        },
    ];

    return (
        <AppLayout breadcrumbs={breadcrumbs} userRole={userRole}>
            <Head title="Lihat Riwayat Peminjaman Kelas" />
            <div className="flex h-full w-full flex-1 flex-col gap-4 rounded-xl p-4 overflow-x-auto">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <h1 className='text-xl py-2 font-semibold'>Lihat Riwayat Peminjaman Kelas</h1>
                    {canUpdate && (
                        <Button variant="outline" asChild>
                            <Link href={`/${userRole}/peminjaman-kelas/riwayat/${peminjamanKelas.id}/ubah`}><Pencil />Ubah</Link>
                        </Button>
                    )}
                </div>
