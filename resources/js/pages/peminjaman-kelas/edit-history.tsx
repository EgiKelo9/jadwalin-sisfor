...
export default function UbahRiwayatPeminjamanKelas({ peminjamanKelasForm, ruangKelas, peminjamanKelas, user, userRole, canDelete }: { peminjamanKelasForm: PeminjamanKelasForm, ruangKelas: RuangKelasData[], peminjamanKelas: PeminjamanKelasData[], user: UserData, userRole: string, canDelete?: boolean }) {
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        {
            title: 'Ubah',
            href: `/${userRole}/peminjaman-kelas/riwayat/${peminjamanKelasForm.id}/ubah`,
        },
    ];

    const { data, setData, post, processing, errors, reset, cancel } = useForm<Required<PeminjamanKelasForm>>({
        id: peminjamanKelasForm.id,
        mahasiswa_id: peminjamanKelasForm.mahasiswa_id || null,
        dosen_id: peminjamanKelasForm.dosen_id || null,
        admin_id: peminjamanKelasForm.admin_id || null,
        ruang_kelas_id: peminjamanKelasForm.ruang_kelas_id,
        tanggal_peminjaman: peminjamanKelasForm.tanggal_peminjaman || '',
        jam_mulai: peminjamanKelasForm.jam_mulai || '',
        jam_selesai: peminjamanKelasForm.jam_selesai || '',
        alasan: peminjamanKelasForm.alasan || '',
        status: peminjamanKelasForm.status || 'pending',
        _method: 'PUT',
    });
    ...
    const handleSubmit: FormEventHandler = (e) => {
        ...
        post(route(`${userRole}.peminjaman-kelas.history.update`, peminjamanKelasForm.id), {
            forceFormData: true,
            preserveState: true,
            preserveScroll: true,
            onSuccess: () => {
                console.log('Update berhasil');
                sessionStorage.setItem('success', 'Data peminjaman kelas berhasil diperbarui.');
                router.visit(route(`${userRole}.peminjaman-kelas.history.edit`, peminjamanKelasForm.id));
            },
            onError: (errors) => {
    ...
    const handleDelete: FormEventHandler = (e) => {
        e.preventDefault();
        setDeleting(true);
        router.delete(route(`${userRole}.peminjaman-kelas.history.destroy`, peminjamanKelasForm.id), {
            onSuccess: () => {
                router.visit(route(`${userRole}.peminjaman-kelas.history.index`));
            },
            onError: (error) => {
                console.error('Gagal menghapus riwayat peminjaman kelas:', error);
            },
            onFinish: () => {
                setDeleting(false);
            },
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs} userRole={userRole}>
            <Head title="Ubah Riwayat Peminjaman Kelas" />
            <ToasterComponent />
            <form className="flex h-full w-full flex-1 flex-col gap-4 rounded-xl p-4 overflow-x-auto" onSubmit={handleSubmit}>
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <h1 className='text-xl py-2 font-semibold'>Ubah Riwayat Peminjaman Kelas</h1>
                    <div className="flex items-center justify-center max-w-sm mb-2 gap-4">
                        <Button variant="outline" asChild>
                            <Link href={`/${userRole}/peminjaman-kelas/riwayat/${peminjamanKelasForm.id}`}><Eye />Lihat</Link>
                        </Button>
                        {canDelete && (
                            <AlertDialog>
                                ...
                                <AlertDialogContent>
                                    <AlertDialogHeader>
                                        <AlertDialogTitle>Apakah Anda yakin ingin menghapus riwayat ini?</AlertDialogTitle>
                                        <AlertDialogDescription>
                                            Aksi ini tidak dapat dibatalkan. Riwayat ini akan dihapus secara permanen
