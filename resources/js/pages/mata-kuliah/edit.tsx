...
export default function UbahMataKuliah({ mataKuliah, dosens, userRole, canDelete }: { mataKuliah: MataKuliahForm, dosens: DosenData[], userRole: string, canDelete?: boolean }) {
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        {
            title: 'Ubah',
            href: `/${userRole}/mata-kuliah/${mataKuliah.id}/ubah`,
        },
    ];
    ...
    const handleDelete: FormEventHandler = (e) => {
        e.preventDefault();
        setDeleting(true);
        router.delete(route(`${userRole}.mata-kuliah.destroy`, mataKuliah.id), {
            onSuccess: () => {
                router.visit(route(`${userRole}.mata-kuliah.index`));
            },
            onError: (error) => {
    ...
    const handleSubmit: FormEventHandler = (e) => {
        ...
        post(route(`${userRole}.mata-kuliah.update`, mataKuliah.id), {
            forceFormData: true,
            preserveState: true,
            preserveScroll: true,
            onSuccess: () => {
                ...
                router.visit(route(`${userRole}.mata-kuliah.edit`, mataKuliah.id));
            },
            onError: (errors) => {
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    ...
                    <div className="flex items-center justify-center max-w-sm mb-2 gap-4">
                        <Button variant="outline" asChild>
                            <Link href={`/${userRole}/mata-kuliah/${mataKuliah.id}`}><Eye />Lihat</Link>
                        </Button>
                        {canDelete && (
                            <AlertDialog>
