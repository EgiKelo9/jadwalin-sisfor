...
export default function UbahRuangKelas({ ruangKelas, userRole, canDelete }: { ruangKelas: RuangKelasForm, userRole: string, canDelete?: boolean }) {
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        {
            title: 'Ubah',
            href: `/${userRole}/ruang-kelas/${ruangKelas.id}/ubah`,
        },
    ];
    ...
    const handleDelete: FormEventHandler = (e) => {
        e.preventDefault();
        setDeleting(true);
        router.delete(route(`${userRole}.ruang-kelas.destroy`, ruangKelas.id), {
            onSuccess: () => {
                router.visit(route(`${userRole}.ruang-kelas.index`));
            },
            onError: (error) => {
    ...
    const handleSubmit: FormEventHandler = (e) => {
        ...
        post(route(`${userRole}.ruang-kelas.update`, ruangKelas.id), {
            forceFormData: true,
            preserveState: true,
            preserveScroll: true,
            onSuccess: () => {
                ...
                router.visit(route(`${userRole}.ruang-kelas.edit`, ruangKelas.id));
            },
            onError: (errors) => {
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    ...
                    <div className="flex items-center justify-center max-w-sm mb-2 gap-4">
                        <Button variant="outline" asChild>
                            <Link href={`/${userRole}/ruang-kelas/${ruangKelas.id}`}><Eye />Lihat</Link>
                        </Button>
                        {canDelete && (
                            <AlertDialog>
